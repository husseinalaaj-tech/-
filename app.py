    ...
    for idx, item in enumerate(results, 1):
        with st.container(border=True):
            ...
            st.text_area(
                ...
                height=80,
                # tab_key يضمن أن نفس النتيجة تحصل على مفتاح مختلف في كل تبويب تظهر فيه
                # (تبويب "الكل" + تبويب المحرك)، لأن Streamlit يبني كل التبويبات في نفس التشغيلة
...
if st.button("🚀 تشغيل البحث المجمع", type="primary", use_container_width=True):
    if not search_query.strip():
        ...
        # تصفير حالة عناصر الترتيب/التصفية من بحث سابق، لتفادي بقاء قيمة
        # (مثل اسم محرك) محفوظة في multiselect بينما لم تعد ضمن الخيارات الجديدة
